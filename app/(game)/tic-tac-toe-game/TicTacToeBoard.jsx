...
export function TicTacToeBoard({ ctx, G, moves, playerID }) {
  ...
  useEffect(() => {
    // Check URL for a refreshed flag.
    const urlParams = new URLSearchParams(window.location.search);
    const hasRefreshed = urlParams.get('refreshed');
    const currentPlayerID = urlParams.get("playerID") || "0";
    ...
    else if (currentPlayerID === "1" && !localStorage.getItem('player1Refreshed')) {
    ...
    if (ctx.gameover) {
      ...
      const leaveMatch = async () => {
        try {
          ...
          const credsParam = urlParams.get("playerCredentials");
          let credentials = credsParam ? JSON.parse(decodeURIComponent(credsParam)) : null;
          if (!credentials) {
          ...
          if (matchID && playerIDFromUrl) {
            console.log(`Leaving match ${matchID} for player ${playerIDFromUrl} with creds:`, credentials);
            const res = await fetch(
              `http://localhost:8001/games/tic-tac-toe/${matchID}/leave`,
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ playerID: playerIDFromUrl, credentials }),
              }
            );
